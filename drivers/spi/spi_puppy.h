#ifndef __SPI_PUPPY_H__
#define __SPI_PUPPY_H__
...
// SPI Command Buffer Size in words
#define SPI_CMD_BUF_SIZE 8
...
// SPI command fields offset, masks and values
...
// #define SPI_CMD_SEND_ADDR_VALUE(value)  ((((value) & 0xff000000) >> 24) | (((value) & 0xff0000)
// >> 8) | (((value) & 0xff00) << 8) | (((value) & 0xff) << 24))
...
#define SPI_CMD_CFG(clockDiv, cpol, cpha)                                                          \
	((SPI_CMD_CFG_ID << SPI_CMD_ID_OFFSET) | ((cpol) << SPI_CMD_CFG_CPOL_OFFSET) |             \
	 ((cpha) << SPI_CMD_CFG_CPHA_OFFSET) | ((clockDiv) << SPI_CMD_CFG_CLK_DIV_OFFSET))

#define SPI_CMD_SOT(cs) ((SPI_CMD_SOT_ID << SPI_CMD_ID_OFFSET) | ((cs) << SPI_CMD_SOT_CS_OFFSET))

#define SPI_CMD_SEND_CMD(cmd, bits, qpi)                                                           \
	((SPI_CMD_SEND_CMD_ID << SPI_CMD_ID_OFFSET) | ((qpi) << SPI_CMD_SEND_CMD_QPI_OFFSET) |     \
...
#define SPI_CMD_DUMMY(cycles)                                                                      \
	((SPI_CMD_DUMMY_ID << SPI_CMD_ID_OFFSET) | (((cycles) - 1) << SPI_CMD_DUMMY_CYCLE_OFFSET))
...
#define SPI_CMD_SETUP_UCS(txrxen, ds, size)                                                        \
	((SPI_CMD_SETUP_UCS_ID << SPI_CMD_ID_OFFSET) |                                             \
	 ((txrxen) << SPI_CMD_SETUP_UC_TXRXEN_OFFSET) | ((ds) << SPI_CMD_SETUP_UC_DS_OFFSET) |     \
	 (size & 0xFFFF))

#define SPI_CMD_TX_DATA(words, wordstrans, bitsword, qpi, lsbfirst)                                \
	((SPI_CMD_TX_DATA_ID << SPI_CMD_ID_OFFSET) | ((qpi) << SPI_CMD_TX_DATA_QPI_OFFSET) |       \
...
#define SPI_CMD_RX_DATA(words, wordstrans, bitsword, qpi, lsbfirst)                                \
	((SPI_CMD_RX_DATA_ID << SPI_CMD_ID_OFFSET) | ((qpi) << SPI_CMD_RX_DATA_QPI_OFFSET) |       \
...
#define SPI_CMD_RPT(iter)                                                                          \
	((SPI_CMD_RPT_ID << SPI_CMD_ID_OFFSET) | ((iter) << SPI_CMD_RPT_NB_OFFSET))

#define SPI_CMD_EOT(evt, cs_keep)                                                                  \
	((SPI_CMD_EOT_ID << 28) | ((evt) << SPI_CMD_EOT_GEN_EVT_OFFSET) |                          \
	 ((cs_keep) << SPI_CMD_EOT_CS_KEEP_OFFSET))

#define SPI_CMD_RX_CHECK(mode, bits, value, qpi, byte_align)                                       \
	((SPI_CMD_RX_CHECK_ID << SPI_CMD_ID_OFFSET) | ((value) << SPI_CMD_RX_CHECK_VALUE_OFFSET) | \
...
#define SPI_CMD_WAIT(event)                                                                        \
	((SPI_CMD_WAIT_ID << SPI_CMD_ID_OFFSET) | ((event) << SPI_CMD_WAIT_EVENT_OFFSET))

#define SPI_CMD_RPT_END() ((SPI_CMD_RPT_END_ID << SPI_CMD_ID_OFFSET))

#define SPI_CMD_FUL(words, wordstrans, bitsword, lsbfirst)                                         \
	((SPI_CMD_FUL_ID << SPI_CMD_ID_OFFSET) | ((wordstrans) << SPI_CMD_FUL_WORDTRANS_OFFSET) |  \
	 ((bitsword - 1) << SPI_CMD_FUL_BITSWORD_OFFSET) |                                         \
	 (((words) - 1) << SPI_CMD_FUL_SIZE_OFFSET) | ((lsbfirst) << SPI_CMD_FUL_LSBFIRST_OFFSET))

#endif //__SPI_PUPPY_H__
